...
export default function Sidebar({ ideas, userData }) {
  const { username } = useContext(UserContext);

  return (
    <aside className="w-64 shadow-xl" aria-label="Sidebar">
        ...
        >
          ...
          {ideas.map((idea) => (
            <li>
              <a
                href={idea.id}
                className="flex items-center p-2 text-base  text-foreground transition duration-75 rounded-lg hover:bg-comment dark:hover:bg-gray-700 dark:text-white group"
              >
                <span className="ml-4 font-medium">{idea.title}</span>
              </a>
            </li>
          ))}
        </ul>
